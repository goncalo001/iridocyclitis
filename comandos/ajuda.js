...
module.exports.run = async (bot, message, args) => {
    ...
    if(!args[0]) {
        ...
        .addField(`☺️ Diversão:`, "``memes`` ``bola8`` ``matar`` ``pesquisar`` ``gay`` ``sondagem`` ``urban`` ``piada`` ``roast`` ``confissão`` ``photograph`` ``illegal`` ``wanted`` ``beautiful`` ``newchallenger`` ``triggered`` ``yugiohcard`` ``worthless`` ``rip`` ``pedrapapeltesoura`` ``roletarussa``")
