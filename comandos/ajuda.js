...
module.exports.run = async (bot, message, args) => {
    ...
    if(!args[0]) {
        ...
        .addField(`☺️ Diversão:`, "``memes`` ``bola8`` ``matar`` ``pesquisar`` ``gayradar`` ``sondagem`` ``urban`` ``piada`` ``roast`` ``confissão`` ``photograph`` ``illegal`` ``wanted`` ``beautiful`` ``newchallenger`` ``triggered`` ``yugiohcard`` ``worthless`` ``rip`` ``pedrapapeltesoura`` ``roletarussa`` ``ascii`` ``spoiler`` ``ghost`` ``pixelizar`` ``nickname`` ``google`` ``minecraftachievement`` ``ytcanalpesquisar``")
        .addField(`⚒️ Utíl:`, "``infoservidor`` ``infousuário`` ``ajuda`` ``metereologia`` ``github``")
