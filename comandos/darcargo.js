...
module.exports.run = async (bot, message, args, ops) => {
    ...
    if(!rMember) return message.channel.send("É necessário mencionar o nome de um usuário para que eu lhe possa atribuir um cargo.").then(m => m.delete(2000));
    ...
    if(!role) return message.channel.send("É necessário fornecer o nome de um cargo existente para eu atribuir ao usuário.").then(m => m.delete(2000));

    if(rMember.roles.has(role.id)) {
        return message.channel.send(`O usuário ${rMember.displayName}, já tem esse cargo.`).then(m => m.delete(2000));
    } else {
        await rMember.addRole(role.id).catch(e => console.log(e.message))
        message.channel.send(`O cargo ${role.name}, foi atribuído a ${rMember.displayName}`).then(m => m.delete(2000));
    }

    let embed = new Discord.RichEmbed()
    .setColor(cores.azul)
    .setAuthor(`${message.guild.name} Modlogs`, message.guild.iconURL)
    .addField("Moderação:", "darcargo")
